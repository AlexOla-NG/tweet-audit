...
Python was chosen for its excellent standard library (JSON/CSV handling) and the official `google-genai` SDK. We migrated to the new **`google-genai` SDK** (replacing the deprecated `google-generativeai`) and upgraded to **Gemini 2.0 Flash Lite** (from 2.5 Flash) to mitigate severe daily rate limits on the free tier. Lite provides a much higher quota (1,500 requests/day vs 20) while remaining capable for batch classification tasks. We kept dependencies minimal (`pytest` for testing, `google-genai` for AI) to ensure the tool remains lightweight.
